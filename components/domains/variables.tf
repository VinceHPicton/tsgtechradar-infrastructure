...
variable "group" {
  ...
  validation {
    condition     = var.group == "prod"
    error_message = "This component should only be deployed to the prod group"
  }
}
...
variable "environment" {
  ...
  validation {
    condition     = var.environment == "prod01"
    error_message = "This component should only be deployed to the prod01 environment"
  }
}
...
variable "dev_aws_account_id" {
  type        = string
  description = "The Dev AWS Account ID (numeric)"
}
